      ...
      var webApi = {
        ...
        SzOpenData : { 
        PullData: '/api/SzOpenData/PullData?sjsybh={sjsybh}', //数据索引编号; 方式:POST; 是否需要token:需要；参数说明:[sjsybh=》数据索引编号；]
        GetExDataIndex: '/api/SzOpenData/GetExDataIndex?keywords={keywords}&pageNum={pageNum}&pageSize={pageSize}', //获取外部数据索引; 方式:GET; 是否需要token:需要；参数说明:[keywords=》关键字；pageNum=》页号；pageSize=》页大小；]
        },
        //文本分析接口。根据方法设置token
        TextAnalysis : { 
