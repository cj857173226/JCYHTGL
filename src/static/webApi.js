...
SzOpenData : { 
      ...
      GetData: '/api/SzOpenData/GetData?sjsybh={sjsybh}&p={p}&ps={ps}', //; 方式:GET; 是否需要token:需要；参数说明:[sjsybh=》数据索引编号；p=》未注释；ps=》未注释；]
      UpdateData: '/api/SzOpenData/UpdateData', //更新数据; 方式:POST; 是否需要token:需要；参数说明:[data=》数据；]
      },
      //文本分析接口。根据方法设置token
      TextAnalysis : { 
