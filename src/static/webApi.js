var webApi = {
  ...
  GetAll: '/api/WebData/GetAll?rqlx={rqlx}&qsrq={qsrq}&jzrq={jzrq}&order={order}&province={province}&city={city}&county={county}&keyword={keyword}&type1={type1}&type2={type2}&sfzsj={sfzsj}&sfzc={sfzc}&clr={clr}&site={site}&p={p}&ps={ps}', //获取互联网线索集合; 方式:GET; 是否需要token:需要；参数说明:[rqlx=》日期类型；qsrq=》起始日期；jzrq=》截至日期；order=》排序字段，默认为采集日期；province=》省份；city=》地市；county=》区县；keyword=》关键词；type1=》数据大类；type2=》线索类型；sfzsj=》是否脏数据；sfzc=》是否暂存；clr=》处理人；site=》采集站点名称；p=》页码；ps=》页面大小，最大100；]
